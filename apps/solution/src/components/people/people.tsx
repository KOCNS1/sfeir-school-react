  ...
  const filteredPeople = useMemo(
    () =>
      people?.filter((person) => {
        ...
        return (
          normalize(person.lastname).includes(searchTermNormalized) ||
          normalize(person.firstname).includes(searchTermNormalized)
        );
      }),
    [people, searchTerm]
  );
...
function SearchBar({ searchTerm, onSearchTermChange }: SearchBarProps) {
  return (
    ...
    >
      <label className={style.srOnly} htmlFor="searchBar">
        Search person
      </label>
      <input
        type="text"
